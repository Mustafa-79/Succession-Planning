...
const registerUser = async (reqs, resp) => {
    try {
        ...
        const { name, email, password, empID, s_img, phone, dob, education, certifications, awards, profilePicture, question, answer } = reqs.body;
        ...
        const updatedUser = await Employee.findOneAndUpdate({ employeeID: empID }, { email: email, password: hashedPassword, two_factor_answer: s_img, gender: user.gender, contactNumber: phone, date_of_birth: dob, education: education, certifications: certifications, awards: awards, profile_picture: profilePicture, security_question: question, security_answer: answer, registered_status: true }, { new: true, runValidators: true })
...
// a function to assign mentor to the employee and set the state
